        ...
        <div class="flex h-screen overflow-hidden bg-slate-50">
            ...
            <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-slate-200 flex flex-col transform -translate-x-full transition-transform duration-300 lg:relative lg:translate-x-0 shrink-0 bg-[radial-gradient(#cbd5e1_1px,transparent_1px)] [background-size:16px_16px] bg-[position:0_0,8px_8px]">
                ...
                <nav class="flex-1 overflow-y-auto p-4 space-y-4 z-10 relative">
                    ...
                    <!-- PENGATURAN -->
                    <div class="pt-4">
                        <p class="px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Pengaturan</p>

                        <a href="{{ route('profile.edit') }}"
                        class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition {{ request()->routeIs('profile.edit') ? 'bg-teal-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100' }}">
            ...
            <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-50/50">
                ...
                <header class="h-16 shrink-0 bg-teal-600 flex items-center justify-between px-4 sm:px-6 z-20 shadow-md">
                    ...
                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 p-1.5 pr-3 rounded-full hover:bg-white/10 transition cursor-pointer text-white">
                        <span class="text-sm font-semibold tracking-wide hidden sm:inline">
                            {{ Auth::user()->name }}
                        </span>
                        <div class="w-9 h-9 rounded-full bg-white text-teal-700 font-bold flex items-center justify-center shadow-sm">
