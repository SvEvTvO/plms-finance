        ...
        <form action="{{ route('categories.update', $category) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    <label for="icon" class="block text-sm font-medium text-heading mb-1">Kode Ikon</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <i class="ti {{ $category->icon ?? 'ti-mood-smile' }} text-muted text-lg"></i>
                        </div>
                        <input type="text" name="icon" id="icon" class="w-full pl-10 rounded-lg border-border focus:border-primary focus:ring-primary/20 text-body bg-background py-2.5" value="{{ old('icon', $category->icon) }}">
                    </div>
