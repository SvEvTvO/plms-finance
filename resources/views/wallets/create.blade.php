<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            ...
            <h2 class="text-2xl font-heading font-bold text-heading">Tambah Wallet Baru</h2>
        </div>
    </x-slot>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" id="name" required class="w-full pl-10 rounded-lg border-border focus:border-primary focus:ring-primary/20 text-body bg-background py-2.5" placeholder="Misal: BCA, GoPay, Dompet Tunai" value="{{ old('name') }}">
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="type" id="type" required class="w-full pl-10 rounded-lg border-border focus:border-primary focus:ring-primary/20 text-body bg-background py-2.5 appearance-none">
                            ...
                            <option value="Cash" {{ old('type') == 'Cash' ? 'selected' : '' }}>Tunai / Cash</option>
                ...
                <!-- Kolom 3: Saldo Awal -->
                <div>
                    <label for="balance" class="block text-sm font-medium text-heading mb-1">Saldo Awal</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <span class="text-muted font-bold">Rp</span>
                        </div>
                        <input type="number" name="balance" id="balance" required min="0" step="0.01" class="w-full pl-10 rounded-lg border-border focus:border-primary focus:ring-primary/20 text-body bg-background py-2.5 font-medium" placeholder="0" value="{{ old('balance', 0) }}">
                    </div>
                    <p class="mt-1 text-xs text-muted">Pastikan nominal sesuai. Saldo awal menentukan perhitungan awal kas Anda.</p>
                    @error('balance') <p class="mt-1 text-sm text-danger flex items-center"><i class="ti ti-alert-circle mr-1"></i>{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-3 bg-background p-1.5 rounded-lg border border-border">
                        <input type="color" name="color" id="color" class="h-10 w-16 p-0.5 rounded cursor-pointer border-0 bg-transparent" value="{{ old('color', '#14B8A6') }}">
                        <span class="text-sm text-muted">Pilih warna untuk identifikasi dompet.</span>
                    </div>
