<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            ...
            <h2 class="text-2xl font-heading font-bold text-heading">Edit Transaksi</h2>
        </div>
    </x-slot>
    ...
    <!-- Alert Peringatan Update Transaksi -->
    <div class="mb-6 p-4 bg-warning/10 border border-warning/20 text-warning-800 rounded-xl flex items-center">
        <i class="ti ti-alert-triangle w-5 h-5 mr-2 flex-shrink-0 text-warning"></i>
        <span class="text-sm">Menyimpan perubahan akan otomatis membatalkan (reverse) efek transaksi lama, lalu menerapkan nominal yang baru ke saldo dompet Anda.</span>
    </div>
    ...
    <div class="bg-surface rounded-xl border border-border p-6 shadow-sm w-full">
        <form action="{{ route('transactions.update', $transaction) }}" method="POST">
            ...
            <div class="space-y-8">
                ...
                <!-- 1. Pemilihan Tipe Transaksi Berwarna -->
                <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="flex shadow-sm rounded-lg">
                            ...
                            <input type="date" name="date" id="date" required class="flex-1 min-w-0 block w-full px-3 py-2.5 rounded-none rounded-r-lg border-border focus:border-primary focus:ring-primary/20 text-body" value="{{ old('date', $transaction->date ? $transaction->date->format('Y-m-d') : '') }}">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="flex shadow-sm rounded-lg">
                            ...
                            <input type="number" name="amount" id="amount" required min="1" step="0.01" class="flex-1 min-w-0 block w-full px-3 py-2.5 rounded-none rounded-r-lg border-border focus:border-primary focus:ring-primary/20 text-body font-medium text-lg" value="{{ old('amount', floatval($transaction->amount)) }}">
                        </div>
                    ...
                    <div id="area-transfer" class="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 hidden">
                        <div>
                            ...
                            <select name="source_wallet_id" id="source_wallet_id" class="w-full rounded-lg border-danger/30 focus:border-danger focus:ring-danger/20 text-body bg-background py-2.5">
                                ...
                                @foreach($allWallets as $wallet)
                                    <option value="{{ $wallet->id }}" {{ old('source_wallet_id', $transaction->source_wallet_id) == $wallet->id ? 'selected' : '' }}>
                                        {{ $wallet->name }} (Saldo: Rp {{ number_format($wallet->balance, 0, ',', '.') }})
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="2" class="w-full rounded-lg border-border focus:border-primary focus:ring-primary/20 text-body bg-background py-2.5">{{ old('description', $transaction->description) }}</textarea>
